...
// ═══════════════════════════════════════════════════════════════════════════
const ACTIVITIES = [
  { icon: '💰', text: 'Sarah hired a Web Developer', color: 'text-emerald-400' },
  { icon: '✅', text: 'Project "E-commerce App" completed', color: 'text-blue-400' },
  { icon: '🎉', text: 'New freelancer joined from Mumbai', color: 'text-violet-400' },
  { icon: '⚡', text: 'Payment of ₹45,000 released', color: 'text-amber-400' },
  { icon: '🚀', text: 'John found a designer in 2 min', color: 'text-cyan-400' },
  { icon: '⭐', text: 'Priya received 5-star rating', color: 'text-yellow-400' },
  { icon: '🤝', text: 'New escrow protection enabled', color: 'text-emerald-400' },
  { icon: '💬', text: 'Real-time chat launched', color: 'text-blue-400' },
]
  ...
  return (
    <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-2xl p-4">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <span className="text-xl">{activities[currentIndex]?.icon}</span>
          <p className={`text-sm font-medium ${activities[currentIndex]?.color}`}>
...
const TrustBar = memo(function TrustBar({ stats }) {
  const statItems = [
    { value: stats?.freelancers || '50K+', label: 'Active Freelancers', icon: '👨‍💻' },
    { value: stats?.projects || '100K+', label: 'Projects Done', icon: '📋' },
    { value: stats?.earnings || '₹50Cr+', label: 'Earnings Released', icon: '💰' },
    { value: stats?.rating || '4.9/5', label: 'Client Rating', icon: '⭐' },
  ]
...
const CategoryCard = memo(function CategoryCard({ category, index, onClick }) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`
        group relative bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-2xl p-6
        cursor-pointer transition-all duration-300 hover:bg-white/[0.06] hover:border-white/20
        hover:shadow-2xl ${color.glow} ${color.hover}
      `}
    >
      ...
      <div className={`w-14 h-14 bg-gradient-to-br ${color.gradient} rounded-xl flex items-center justify-center mx-auto mb-4 shadow-lg group-hover:shadow-xl transition-all duration-300`}>
      ...
      <div className={`mt-3 inline-flex items-center gap-1 bg-gradient-to-r ${color.gradient} text-white text-[10px] font-bold px-3 py-1 rounded-full mx-auto opacity-0 group-hover:opacity-100 transition-all duration-300`}>
        Explore <span>→</span>
      </div>
    </motion.div>
  )
...
const FreelancerCard = memo(function FreelancerCard({ freelancer, index }) {
  const skills = Array.isArray(freelancer.skills) ? freelancer.skills : (freelancer.skills || '').split(',').slice(0, 3)
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3 mb-4">
        <div className="relative">
          {freelancer.user?.profilePic ? (
            <img src={freelancer.user.profilePic} alt={freelancer.user?.name} className="w-14 h-14 rounded-xl object-cover ring-2 ring-white/20" />
          ) : (
            <div className={`w-14 h-14 rounded-xl bg-gradient-to-br ${gradients[colorIndex]} flex items-center justify-center`}>
              <span className="text-white font-bold text-lg">{(freelancer.user?.name || '?').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}</span>
            </div>
          )}
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {skills.map((sk, i) => (
          <span key={i} className="bg-white/5 border border-white/10 text-white/50 text-[10px] px-2 py-0.5 rounded-md">{sk}</span>
        ))}
      </div>
      <div className="flex items-center justify-between pt-3 border-t border-white/5">
        <div>
          <span className="text-sm font-bold text-white">{freelancer.hourlyRate ? `₹${freelancer.hourlyRate}` : 'Open'}</span>
...
const HowItWorksStep = memo(function HowItWorksStep({ step, index, isActive }) {
  ...
  const icons = [
    <svg key="1" className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>,
    ...
    <svg key="3" className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>,
  ]
  ...
  return (
    ...
    >
      ...
      <span className={`text-xs font-bold uppercase tracking-wider mb-2 ${isActive ? 'text-blue-400' : 'text-white/40'}`}>Step {index + 1}</span>
      ...
      {isActive && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="mt-3 h-1 w-16 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-full"
        />
      )}
    </motion.div>
  )
...
const FeatureCard = memo(function FeatureCard({ feature, index }) {
  return (
    ...
    >
      <div className={`w-12 h-12 bg-gradient-to-br ${feature.gradient} rounded-xl flex items-center justify-center mb-4 shadow-lg group-hover:shadow-xl transition-all duration-300`}>
...
const PricingCard = memo(function PricingCard({ plan, index, isHighlighted }) {
  return (
    <motion.div
      ...
      className={`relative bg-white/[0.03] backdrop-blur-xl border rounded-2xl p-6 transition-all duration-300 ${isHighlighted ? 'border-blue-500/50 bg-gradient-to-b from-blue-500/10 to-transparent' : 'border-white/10 hover:bg-white/[0.06] hover:border-white/20'}`}
    >
      {isHighlighted && (
      ...
      <Link
        to="/register"
        className={`block text-center py-2.5 rounded-xl font-bold text-sm transition-all duration-200 ${isHighlighted ? 'bg-gradient-to-r from-blue-500 to-cyan-500 text-white hover:shadow-lg hover:shadow-blue-500/30' : 'bg-white/5 text-white/70 border border-white/10 hover:bg-white/10 hover:border-white/20'}`}
      >
        {plan.cta}
      </Link>
    </motion.div>
...
const Home = () => {
  ...
  const categories = [
    { key: 'web', label: 'Web Dev', icon: '🌐', count: '5,200+' },
    { key: 'design', label: 'UI/UX Design', icon: '🎨', count: '3,800+' },
    { key: 'ai', label: 'AI / ML', icon: '🤖', count: '1,500+' },
    { key: 'mobile', label: 'Mobile Dev', icon: '📱', count: '2,200+' },
    { key: 'data', label: 'Data Science', icon: '📊', count: '1,100+' },
    { key: 'writing', label: 'Content', icon: '✍️', count: '890+' },
  ]
  ...
  const features = [
    { icon: '🔒', title: 'Secure Escrow Payments', desc: 'Your money is protected until you approve the work. Fully refundable if unsatisfied.', gradient: 'from-emerald-500 to-teal-500' },
    { icon: '💬', title: 'Real-time Chat', desc: 'Communicate instantly with freelancers. Share files, track progress, and get updates.', gradient: 'from-blue-500 to-cyan-500' },
    { icon: '⚡', title: 'Fast Delivery', desc: 'Get quality work delivered on time. Our freelancers are vetted for professionalism.', gradient: 'from-amber-500 to-orange-500' },
    { icon: '✅', title: 'Verified Freelancers', desc: 'All freelancers are identity-verified. Read reviews and hire with confidence.', gradient: 'from-violet-500 to-purple-500' },
  ]
  ...
  const pricingPlans = [
    { icon: '👤', title: 'Client', subtitle: 'For clients hiring freelancers', price: 'FREE', period: '', gradient: 'from-slate-500 to-gray-500', features: ['Post unlimited projects', 'Browse all freelancers', 'Real-time chat', 'Secure escrow payments', '24/7 support'], cta: 'Get Started' },
    { icon: '💼', title: 'Starter', subtitle: 'For new freelancers', price: '₹299', period: '/month', gradient: 'from-blue-500 to-cyan-500', features: ['5 project proposals/month', 'Featured profile', 'Priority support', 'Basic analytics', 'Email notifications'], cta: 'Start Freelancing' },
    { icon: '⭐', title: 'Pro', subtitle: 'For serious freelancers', price: '₹499', period: '/month', gradient: 'from-violet-500 to-purple-500', features: ['Unlimited proposals', 'Top placement in search', 'Advanced analytics', 'Dedicated support', 'Skill certifications'], cta: 'Go Pro' },
  ]
    ...
    const fetchData = async () => {
      try {
        const [freelancersRes] = await Promise.all([
          // ✅ FIX: /api/freelancers → /api/freelancer (singular)
          // ✅ FIX: port 5000 → 5050 (tera backend 5050 pe hai)
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-8">
              ...
              >
                ...
                <span className="text-xs sm:text-sm text-white/70 font-medium">10,000+ verified experts online now</span>
              </motion.div>
              ...
              >
                <Link to="/talent" className="bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-6 py-3 rounded-xl font-semibold text-sm hover:shadow-lg hover:shadow-blue-500/30 hover:scale-105 transition-all duration-200">
                  Find Talent ↗
                </Link>
                <Link to="/find/work" className="bg-white/[0.05] text-white px-6 py-3 rounded-xl font-semibold text-sm border border-white/10 hover:bg-white/[0.08] hover:border-white/20 transition-all duration-200">
                  Find Work
                </Link>
              </motion.div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {loading ? (
              ...
              freelancers.slice(0, 8).map((freelancer, i) => (
                <FreelancerCard key={freelancer._id} freelancer={freelancer} index={i} />
              ))
            ) : (
      ...
      <section className="relative z-10 py-20 bg-gradient-to-r from-blue-600 via-violet-600 to-purple-600 overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-white/80 text-lg mb-8 max-w-lg mx-auto">
            Join 50,000+ businesses and freelancers working together on Co.Worker.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/register" className="bg-white text-blue-600 px-8 py-4 rounded-xl font-bold text-base hover:shadow-2xl hover:scale-105 transition-all duration-200">
              Post a Project →
            </Link>
            <Link to="/find/work" className="bg-white/10 text-white px-8 py-4 rounded-xl font-bold text-base border-2 border-white/30 hover:bg-white/20 transition-all duration-200">
              Find Work
            </Link>
          </div>
        </div>
      ...
      <footer className="relative z-10 bg-[#0a0f1a] py-12 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {[
              ...
              <div key={col.title}>
                ...
                <ul className="space-y-2">
                  {col.links.map(l => <li key={l}><a className="text-white/40 hover:text-white text-sm transition-colors duration-200 cursor-pointer">{l}</a></li>)}
                </ul>
              </div>
            ))}
          </div>
          <div className="border-t border-white/5 pt-8 flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="text-sm text-white/30 mb-4 md:mb-0">© 2024 Co.Worker. All rights reserved.</div>
            <div className="flex gap-6">
              {['Privacy', 'Terms', 'Security'].map(l => <a key={l} className="text-white/30 hover:text-white text-sm transition-colors duration-200 cursor-pointer">{l}</a>)}
            </div>
          </div>
        </div>
      </footer>
