...
import { fetchRatingSummary } from "../features/rating/ratingSlice"
import {
    BadgeCheck,
    BriefcaseBusiness,
    Compass,
...
const ProjectModal = ({ project, onClose, onBidStatusChange }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm"
            ...
            <div className="modal-in w-full max-w-3xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 dark:border-slate-700 dark:bg-slate-950 dark:shadow-black/50">
                ...
                <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-6 py-5 relative overflow-hidden">
                    ...
                    <button onClick={onClose}
                        className="absolute right-4 top-4 flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border-none bg-white/20 text-lg font-bold text-white transition hover:bg-white/35 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70">✕</button>
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-bold ${STATUS_COLORS[project.status] || STATUS_FALLBACK}`}>
                            {project.status}
                        </span>
                ...
                <div className="grid grid-cols-2 gap-2 border-b border-slate-200 bg-slate-100 p-2 dark:border-slate-800 dark:bg-slate-900">
                    {[{ id: "details", label: "📋 Details" }, { id: "bids", label: `💼 Bids (${bids.length})` }].map(t => (
                        <button key={t.id} onClick={() => setActiveView(t.id)}
                            className={`cursor-pointer rounded-2xl border px-3 py-3 text-sm font-bold transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/50 ${activeView === t.id
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <div key={item.label} className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-slate-700 dark:bg-slate-900">
                                    ...
                                    <p className={`text-sm font-black ${item.color} dark:text-cyan-200`}>{item.value}</p>
                                </div>
                            ))}
                        </div>
                        {project.description && (
                            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900">
                                <p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-400">Description</p>
                    ...
                    <div className="max-h-[55vh] overflow-y-auto">
                        {bidsLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-16 text-center px-6">
                                ...
                                <p className="mb-1 text-base font-bold text-slate-700 dark:text-slate-100">No bids yet</p>
                                <p className="text-sm text-slate-400">Freelancers haven't applied yet.</p>
                            </div>
                        ) : (
                            <div className="divide-y divide-slate-100 dark:divide-slate-800">
                                {bids.map((bid, i) => {
                                    ...
                                    return (
                                        <div key={bid._id || i} className="fade-up px-5 py-4 transition hover:bg-slate-50 dark:hover:bg-slate-900"
                                            ...
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 flex-wrap">
                                                        <p className="text-sm font-bold text-slate-950 dark:text-slate-100">{fname}</p>
                                                        <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${BID_STATUS_COLORS[bid.status] || STATUS_FALLBACK}`}>
                                                            {bid.status || 'Pending'}
                                                        </span>
                                                    </div>
                                                    <p className="truncate text-xs text-slate-500 dark:text-slate-400">{femail}</p>
                                                    {freelancer?.category && <p className="mt-0.5 text-xs font-medium text-blue-600 dark:text-cyan-300">{freelancer.category}</p>}
                                                    {freelancer?.experience && <p className="text-xs text-slate-400 dark:text-slate-500">{freelancer.experience} yrs experience</p>}
                                                </div>
                                                <div className="text-right flex-shrink-0">
                                                    <p className="text-lg font-black text-emerald-600">₹{bid.amount ? Number(bid.amount).toLocaleString('en-IN') : '—'}</p>
                                                    <p className="text-[10px] text-slate-400 dark:text-slate-500">bid amount</p>
                                                    {bid.createdAt && <p className="mt-0.5 text-[10px] text-slate-400 dark:text-slate-500">{new Date(bid.createdAt).toLocaleDateString('en-IN')}</p>}
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2 mt-3 flex-wrap">
                                                {freelancer?.user?._id && (
                                                    <Link to={`/profile/${freelancer.user._id}`} onClick={onClose}
                                                        className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-1.5 text-xs font-bold text-blue-600 transition-all hover:bg-blue-600 hover:text-white dark:border-blue-300/20 dark:bg-blue-400/10 dark:text-cyan-200 dark:hover:bg-blue-500 dark:hover:text-white">
                                                        👤 View Profile
                                                    </Link>
                                                )}
                                                {[
                                                    ...
                                                    <button key={btn.s}
                                                        ...
                                                        className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all border
                                                            ${bid.status === btn.s ? btn.done + ' cursor-default' : 'bg-white dark:bg-slate-900 dark:border-slate-700 ' + btn.idle + ' ' + btn.hover + ' cursor-pointer'}
                ...
                <div className="flex gap-3 border-t border-slate-200 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900">
                    <button onClick={onClose}
                        className="flex-1 cursor-pointer rounded-2xl border border-slate-200 bg-white py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700">Close</button>
...
const SURFACE = "rounded-[28px] border border-slate-200 bg-white shadow-[0_18px_60px_-34px_rgba(15,23,42,0.22)] dark:border-white/10 dark:bg-white/[0.04] dark:shadow-none"
...
const ProfileMetricCard = ({ icon, label, value, accent = "from-blue-500 to-cyan-500" }) => (
    <div className="rounded-[24px] border border-slate-200 bg-white/82 p-4 shadow-[0_18px_45px_-34px_rgba(15,23,42,0.35)] backdrop-blur dark:border-white/10 dark:bg-white/[0.08] dark:shadow-none">
        ...
        <div className="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-white/45">{label}</div>
    </div>
)

const ProfileInfoCard = ({ icon, title, value, tone = "text-slate-900" }) => (
    <div className="rounded-[22px] border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/[0.05]">
        <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-slate-700 shadow-sm dark:bg-white/[0.07] dark:text-white/70 dark:shadow-none">
                {icon}
            </div>
            <div className="min-w-0">
                <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-white/35">{title}</div>
                <div className={`mt-1 truncate text-sm font-semibold dark:text-white/75 ${tone}`}>{value}</div>
            </div>
        </div>
    </div>
)
...
const ProfileEmptyState = ({ icon, title, message, action }) => (
    <div className={`${SURFACE} p-8 text-center sm:p-10`}>
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-[22px] bg-slate-100 text-slate-600 dark:bg-white/[0.06] dark:text-white/70">
            {icon}
        </div>
...
const PostedProjectCard = ({ project, index, onView }) => (
    ...
    >
        <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                    <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold text-slate-600 dark:border-white/10 dark:bg-white/[0.06] dark:text-white/65">
                ...
                <h3 className="text-base font-black leading-snug text-slate-950 line-clamp-2 dark:text-white">{project.title}</h3>
            </div>

            <button
                onClick={() => onView(project)}
                className="rounded-2xl bg-slate-900 px-4 py-2 text-xs font-bold text-white transition hover:bg-slate-800 dark:bg-gradient-to-r dark:from-blue-500 dark:to-cyan-500"
            >
                View
            </button>
        </div>
        ...
        {project.description && (
            <p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-600 dark:text-white/55">{project.description}</p>
        )}

        <div className="mt-4 grid grid-cols-2 gap-3 rounded-[22px] border border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/[0.05]">
            <div>
                <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-white/35">Budget</div>
                <div className="mt-1 text-sm font-black text-emerald-600">
                    ₹{project.budget ? Number(project.budget).toLocaleString('en-IN') : '—'}
                </div>
            </div>
            <div>
                <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400 dark:text-white/35">Posted</div>
                <div className="mt-1 text-sm font-semibold text-slate-700 dark:text-white/70">
...
const UserProfilePage = () => {
    ...
    const { averageRating, totalReviews } = useSelector(state => state.rating)
    const dispatch = useDispatch()
    ...
    useEffect(() => {
        if (user?._id) {
            ...
            dispatch(fetchRatingSummary({ userId: user._id }))
        }
    }, [dispatch, user?._id])
    ...
    const freelancerProfileId = freelancer?.profile?._id
    const activeFreelancerProjects = Array.isArray(listedProjects) && freelancerProfileId
        ? listedProjects.filter(project => {
            const status = String(project.status || "").toLowerCase()
            return idsMatch(project.freelancer, freelancerProfileId) &&
                ["accepted", "in-progress", "completed"].includes(status)
        })
        : []
    const clientCount = new Set(
        activeFreelancerProjects
            .map(project => getEntityId(project.user))
            .filter(Boolean)
            .map(String)
    ).size
    const reviewCount = getCollectionCount(null, totalReviews)
    const ratingAverage = Number(averageRating || 0)
    ...
    const metricItems = inFreelancer
        ? [
            ...
            { label: "Reviews", value: reviewCount, icon: <Sparkles size={16} />, accent: "from-violet-500 to-fuchsia-500" },
            { label: "Clients", value: clientCount, icon: <BadgeCheck size={16} />, accent: "from-emerald-500 to-teal-500" },
            { label: "Rating", value: `${ratingAverage.toFixed(1)} ★`, icon: <Compass size={16} />, accent: "from-amber-500 to-orange-500" },
        ]
        : [
            { label: "Projects", value: myProjects.length, icon: <BriefcaseBusiness size={16} />, accent: "from-blue-500 to-cyan-500" },
            { label: "Following", value: followingCount, icon: <UserRound size={16} />, accent: "from-violet-500 to-fuchsia-500" },
            { label: "Followers", value: followersCount, icon: <BadgeCheck size={16} />, accent: "from-rose-500 to-orange-500" },
            { label: "Blogs", value: blogCount, icon: <LayoutDashboard size={16} />, accent: "from-emerald-500 to-teal-500" },
        ]
    ...
    return (
        <div style={{ fontFamily: "'DM Sans', sans-serif" }}>
            ...
            <div className="min-h-screen bg-[#f5f7fb] text-slate-900 dark:bg-[#020617] dark:text-white">

                {user?.isFreelancer || justBecameFreelancer ? (
                    <div className="border-b border-slate-200 bg-white/95 dark:border-white/10 dark:bg-[#020617]/95">
                        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-center gap-3 px-4 py-3 text-xs sm:justify-between sm:px-6">
                            <span className="font-semibold text-slate-500 dark:text-white/45">Preview mode: switch between freelancer and regular user views.</span>
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={`rounded-full px-4 py-2 font-bold transition ${inFreelancer ? "bg-slate-900 text-white dark:bg-white dark:text-slate-950" : "bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-white/[0.06] dark:text-white/60 dark:hover:bg-white/[0.09]"}`}
                                >
                                    Freelancer
                                </button>
                                <button
                                    ...
                                    className={`rounded-full px-4 py-2 font-bold transition ${!inFreelancer ? "bg-gradient-to-r from-blue-600 to-cyan-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-white/[0.06] dark:text-white/60 dark:hover:bg-white/[0.09]"}`}
                                >
                                    Regular User
                                </button>
                            </div>
                    ...
                    <div className="border-b border-slate-200 bg-white/95 dark:border-white/10 dark:bg-[#020617]/95">
                        <div className="mx-auto flex max-w-6xl items-center justify-center gap-3 px-4 py-3 text-xs sm:px-6">
                            <span className="font-semibold text-slate-500 dark:text-white/45">Preview mode:</span>
                            <span className="rounded-full bg-gradient-to-r from-blue-600 to-cyan-600 px-4 py-2 font-bold text-white">Regular User</span>
                        </div>
                    </div>
                )}

                <section className="relative overflow-hidden bg-[linear-gradient(180deg,#f8fbff_0%,#eef7ff_52%,#f5f7fb_100%)] pb-10 pt-6 dark:bg-[#071429] sm:pt-8">
                    <div className="pointer-events-none absolute inset-0 overflow-hidden">
                        ...
                        />
                        <div className="absolute inset-0 opacity-[0.6] dark:opacity-[0.06]" style={{ backgroundImage: "linear-gradient(rgba(148,163,184,.14) 1px,transparent 1px),linear-gradient(90deg,rgba(148,163,184,.14) 1px,transparent 1px)", backgroundSize: "44px 44px" }} />
                    </div>

                    <div className="relative mx-auto max-w-6xl px-4 sm:px-6">
                        <div className="rounded-[32px] border border-slate-200 bg-white/88 p-5 shadow-[0_28px_90px_-58px_rgba(15,23,42,0.45)] backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.08] dark:shadow-[0_24px_80px_-40px_rgba(8,145,178,0.4)] sm:p-8">
                            <div className="grid gap-6 lg:grid-cols-[auto,minmax(0,1fr),auto] lg:items-end">
                                <div className="relative mx-auto lg:mx-0">
                                    <img
                                        ...
                                        className="h-24 w-24 rounded-[26px] object-cover ring-4 ring-white shadow-2xl dark:ring-white/20 sm:h-28 sm:w-28"
                                        alt="avatar"
                                    />
                                    <span className={`absolute -bottom-2 -right-2 rounded-full px-3 py-1 text-[11px] font-black shadow-lg ${inFreelancer ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "bg-gradient-to-r from-blue-600 to-cyan-600 text-white"}`}>
                                        {inFreelancer ? "✦ PRO" : "Member"}
                                    </span>
                                </div>

                                <div className="min-w-0 text-center lg:text-left">
                                    <div className="inline-flex items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-[11px] font-bold uppercase tracking-[0.24em] text-cyan-700 dark:border-white/10 dark:bg-white/[0.05] dark:text-cyan-200">
                                        <Sparkles size={12} />
                                        {inFreelancer ? "Freelancer preview" : "Regular user profile"}
                                    </div>
                                    <h1 className="mt-4 text-3xl font-black leading-tight text-slate-950 dark:text-white sm:text-4xl" style={{ fontFamily: "'Playfair Display',serif" }}>
                                        {user?.name}
                                    </h1>
                                    <p className="line-clamp-5 mx-auto mt-2 max-w-2xl text-sm leading-7 text-slate-600 dark:text-white/70 sm:text-base lg:mx-0">
                                        {inFreelancer
                                    ...
                                    <div className="mt-4 flex flex-wrap items-center justify-center gap-2 text-xs font-semibold text-slate-600 dark:text-white/65 lg:justify-start">
                                        <span className="rounded-full border border-slate-200 bg-white px-3 py-1.5 dark:border-white/10 dark:bg-white/[0.05]">{user?.email}</span>
                                        <span className="rounded-full border border-slate-200 bg-white px-3 py-1.5 dark:border-white/10 dark:bg-white/[0.05]">ID: {user?._id?.slice(-8)}</span>
                                        {freelancer?.profile?.category && (
                                            <span className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-cyan-700 dark:border-cyan-400/20 dark:bg-cyan-400/10 dark:text-cyan-200">
                                                {freelancer.profile.category}
                                            </span>
                                        )}
                                    </div>
                                ...
                                <div className="flex flex-col gap-3 sm:flex-row lg:flex-col xl:flex-row">
                                    <button
                                        ...
                                        className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-900 shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-50 dark:border-white/10 dark:bg-white dark:hover:bg-slate-100"
                                    >
                                        <PencilLine size={16} />
                                        Edit Profile
                                    </button>
                ...
                <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8">
                    <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr),minmax(280px,0.8fr)]">
                        <section className={`${SURFACE} p-5 sm:p-6`}>
                            ...
                            <h2 className="mt-3 text-2xl font-black text-slate-950 dark:text-white" style={{ fontFamily: "'Playfair Display',serif" }}>
                            ...
                            <div className="mt-5 flex flex-wrap gap-2.5">
                                {skillTags.slice(0, 8).map((tag) => (
                                    <span key={tag} className="rounded-full border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 dark:border-white/10 dark:bg-white/[0.05] dark:text-white/65">
                                        {tag}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="mt-6 flex flex-wrap gap-2 rounded-[26px] border border-slate-200 bg-white p-2 shadow-[0_16px_50px_-34px_rgba(15,23,42,0.16)] dark:border-white/10 dark:bg-white/[0.04] dark:shadow-none">
                        {(inFreelancer
                            ...
                            <button
                                ...
                                className={`rounded-2xl px-4 py-3 text-sm font-bold transition ${activeTab === tab.id ? "bg-slate-900 text-white dark:bg-white dark:text-slate-950" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900 dark:text-white/55 dark:hover:bg-white/[0.06] dark:hover:text-white"}`}
                            >
                                {tab.label}
                            </button>
                        ))}
                            ...
                            <section className={`${SURFACE} p-5 sm:p-6`}>
                                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        ...
                                        <h2 className="mt-2 text-2xl font-black text-slate-950 dark:text-white" style={{ fontFamily: "'Playfair Display',serif" }}>Previous Work</h2>
                                        <p className="mt-1 text-sm text-slate-500 dark:text-white/45">{freelancer?.previousWorks?.length ?? 0} projects visible to clients</p>
                                    </div>
                                    <button
                                        onClick={openAdd}
                                        className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-3 text-sm font-black text-white transition hover:-translate-y-0.5"
                                ...
                                {!freelancer?.previousWorks?.length ? (
                                    <div className="mt-5 rounded-[26px] border border-dashed border-slate-200 bg-slate-50/60 px-6 py-14 text-center dark:border-white/10 dark:bg-white/[0.05]">
                                        <div className="text-5xl opacity-40">🗂</div>
                                        <p className="mt-4 text-sm font-semibold text-slate-600 dark:text-white/55">No projects yet. Add your first one.</p>
                                    </div>
                                ) : (
                                    <div className="mt-6 grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                                        {freelancer.previousWorks.map((proj) => (
                                            <div key={proj._id} className="group overflow-hidden rounded-[26px] border border-slate-200 bg-slate-50/50 shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-white/10 dark:bg-white/[0.05] dark:shadow-none">
                                                <div className="relative h-44 overflow-hidden bg-slate-100 dark:bg-white/[0.06]">
                                                    {proj.projectImage
                                                ...
                                                <div className="p-4">
                                                    ...
                                                    <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600 dark:text-white/55">{proj.projectDescription}</p>
                                                    {proj.projectLink && (
                            ...
                            <section className={`${SURFACE} p-5 sm:p-6`}>
                                <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                    <div>
                                        ...
                                        <h2 className="mt-2 text-2xl font-black text-slate-950 dark:text-white" style={{ fontFamily: "'Playfair Display',serif" }}>My Posted Projects</h2>
                                        <p className="mt-1 text-sm text-slate-500 dark:text-white/45">{myProjects.length} active entries in your board</p>
                                    </div>

                                    <div className="flex flex-col gap-2 sm:flex-row">
                                        <Link to="/browse-projects">
                                            <button className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50 dark:border-white/10 dark:bg-white/[0.05] dark:text-white/70 dark:hover:border-white/20 dark:hover:bg-white/[0.08] dark:hover:text-white">
                                                <Compass size={16} />
                                                Browse Projects
                                            </button>
                                        </Link>
                                ...
                                {myProjects.length === 0 ? (
                                    <div className="mt-6 rounded-[26px] border border-dashed border-slate-200 bg-slate-50/60 px-6 py-14 text-center dark:border-white/10 dark:bg-white/[0.05]">
                                        <div className="text-5xl opacity-40">📋</div>
                                        <p className="mt-4 text-sm font-semibold text-slate-600 dark:text-white/55">No projects posted yet.</p>
                                        <p className="mt-1 text-sm text-slate-500 dark:text-white/40">Open the composer and publish your first project when you are ready.</p>
                                    </div>
                                ) : (
                            ...
                            <section className={`${SURFACE} overflow-hidden`}>
                                ...
                                >
                                    <div>
                                        ...
                                        <h3 className="mt-2 text-xl font-black text-slate-950 dark:text-white" style={{ fontFamily: "'Playfair Display',serif" }}>
                                            Post your next project without cluttering the whole profile
                                        </h3>
                                        <p className="mt-1 text-sm text-slate-500 dark:text-white/45">Open this only when you want to publish something new.</p>
                                    </div>
                                    <span className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-900 text-white dark:bg-gradient-to-r dark:from-blue-500 dark:to-cyan-500">
                                        {composerOpen ? "−" : "+"}
                                    </span>
                                </button>

                                {composerOpen && (
                                    <div className="border-t border-slate-200 bg-slate-50/70 p-3 dark:border-white/10 dark:bg-white/[0.03] sm:p-5">
                                        <ListProject />
                                    </div>
                                )}
                            </section>
                    ...
                    {inFreelancer && activeTab === "reviews" && (
                        <div className="pb-12 pt-6">
                            <section className={`${SURFACE} p-6 sm:p-7`}>
                                ...
                                <h2 className="mt-2 text-2xl font-black text-slate-950 dark:text-white" style={{ fontFamily: "'Playfair Display',serif" }}>Client Reviews</h2>
                                <div className="mt-6">
                                    <RatingSummary
                                        userId={user?._id}
                    ...
                    {!inFreelancer && activeTab === "saved" && (
                        <div className="pb-12 pt-6">
                            <ProfileEmptyState
                                ...
                                action={(
                                    <Link to="/browse-projects">
                                        <button className="mt-6 inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-bold text-white transition hover:bg-slate-800 dark:bg-gradient-to-r dark:from-blue-500 dark:to-cyan-500">
                                            <Compass size={16} />
                                            Explore Projects
                                        </button>
                                    </Link>
                ...
                {showAddModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm">
                        <form onSubmit={handleSubmit} className="modal-in w-full max-w-xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 dark:border-white/10 dark:bg-[#0f172a] dark:shadow-black/40">
                            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-6 py-5 dark:border-white/10 dark:bg-white/[0.04]">
                                <div>
                                    <h2 className="text-xl font-black text-slate-950 dark:text-white" style={{ fontFamily: "'Playfair Display',serif" }}>
                                        {editTarget ? "Edit Project" : "Add New Project"}
                                    </h2>
                                    <p className="mt-1 text-sm text-slate-500 dark:text-white/55">{editTarget ? "Update your project details" : "Showcase your best work"}</p>
                                </div>
                            ...
                            <div className="max-h-[70vh] space-y-5 overflow-y-auto px-6 py-6">
                                {projectImage && (
                                    <div className="h-40 overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 dark:border-white/10 dark:bg-white/[0.04]">
                                ...
                                {[
                                    ...
                                    <div key={field.name}>
                                        <label className="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-slate-500 dark:text-white/50">{field.label}</label>
                                        {field.type === "textarea"
                                            ? <textarea name={field.name} value={field.value} onChange={handleChange} placeholder={field.placeholder} rows={3}
                                                className="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:border-white/10 dark:bg-white/[0.05] dark:text-white dark:placeholder:text-white/35 dark:focus:border-cyan-300/50 dark:focus:ring-cyan-400/20" />
                            ...
                            <div className="flex gap-3 border-t border-slate-100 bg-slate-50/70 px-6 py-4 dark:border-white/10 dark:bg-white/[0.03]">
                                <button type="button" onClick={() => setShowAddModal(false)}
                                ...
                                <button type="submit"
                                    className="flex-1 cursor-pointer rounded-2xl border-none bg-gradient-to-r from-blue-600 to-cyan-500 py-3 text-sm font-bold text-white shadow-lg shadow-blue-500/25 transition hover:-translate-y-0.5">
                ...
                {editProfile && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-sm">
                        <div className="modal-in w-full max-w-xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 dark:border-white/10 dark:bg-[#0f172a] dark:shadow-black/40">
                            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-6 py-5 dark:border-white/10 dark:bg-white/[0.04]">
                                <div>
                                    <h2 className="text-xl font-black text-slate-950 dark:text-white" style={{ fontFamily: "'Playfair Display',serif" }}>Edit Profile</h2>
                                    <p className="mt-1 text-sm text-slate-500 dark:text-white/55">Update your profile details.</p>
                                </div>
                                <button onClick={() => setEditProfile(false)}
                                    className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-2xl border-none bg-slate-200 text-sm font-bold text-slate-600 transition hover:bg-slate-300 hover:text-slate-900 dark:bg-white/10 dark:text-white/65 dark:hover:bg-white/15 dark:hover:text-white">✕</button>
                            </div>
                            <div className="max-h-[68vh] space-y-5 overflow-y-auto px-6 py-6">
                                <div className="flex items-center gap-4">
                                    <img src={user?.profilePic || "https://i.pravatar.cc/150"} alt="Profile" className="h-20 w-20 rounded-2xl object-cover ring-2 ring-blue-300 dark:ring-cyan-300/40" />
                                ...
                                {inFreelancer && (
                                    <div>
                                        <label className="mb-2 block text-xs font-bold uppercase tracking-[0.16em] text-slate-500 dark:text-white/50">Skills <span className="font-normal normal-case tracking-normal text-slate-400 dark:text-white/40">(comma separated)</span></label>
                                        <input defaultValue="React, Tailwind CSS, Figma, Next.js" className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 transition focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 dark:border-white/10 dark:bg-white/[0.05] dark:text-white dark:placeholder:text-white/35 dark:focus:border-cyan-300/50 dark:focus:ring-cyan-400/20" />
                                    </div>
                                )}
                            </div>
                            <div className="flex gap-3 border-t border-slate-100 bg-slate-50/70 px-6 py-4 dark:border-white/10 dark:bg-white/[0.03]">
